...
export function registerTestsHandlers(): void {
    // Load all test cases from Web API
    ipcMain.handle(IPC.TESTS_LOAD, async (_event, projectId: string): Promise<TestCase[]> => {
        if (!projectId) return []
        return trpcQuery<TestCase[]>('testCase.list', { projectId })
    })

    // Save (create) a test case via Web API
    ipcMain.handle(IPC.TESTS_SAVE, async (_event, projectId: string, items: TestCase[]) => {
        // Create each item that doesn't have an ID yet, or update existing ones
        for (const tc of items) {
            if (tc.id) {
                await trpcMutation('testCase.update', {
                    id: tc.id,
                    path: tc.path,
                ...
                await trpcMutation('testCase.create', {
                    projectId,
                    path: tc.path,
                    contentHash: tc.contentHash || '',
                    description: tc.description || '',
    ...
    // Link: search workspace test files for [id] patterns — local (unchanged)
    ipcMain.handle(
        IPC.TESTS_LINK,
